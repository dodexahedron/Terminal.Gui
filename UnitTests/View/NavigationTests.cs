...
public class NavigationTests {
	...
	public NavigationTests (ITestOutputHelper output) => this.output = output;
	...
	public void HotKey_Will_Invoke_KeyPressed_Only_For_The_MostFocused_With_Top_KeyPress_Event ()
	{
		...
		var sb = new StatusBar (new StatusItem [] {
			new (KeyCode.CtrlMask | KeyCode.Q, "~^Q~ Quit", () => sbQuiting = true)
		});
...
// This test is now invalid because `win` is focused, so it will receive the keypress
	...
	public void FocusNext_Does_Not_Throws_If_A_View_Was_Removed_From_The_Collection ()
	{
		...
		var exception = Record.Exception (() => top1.NewKeyDownEvent (new Key (KeyCode.Tab | KeyCode.CtrlMask)));
		Assert.Null (exception);
		Assert.True (removed);
		Assert.Null (view3);
	}
...
└──────────────────┘", output);
		...
		Assert.Equal (0, col);
		Assert.Equal (0, row);
		Assert.Equal (top, View.FindDeepestView (top, 0, 0, out var rx, out var ry));
		Assert.Equal (0, rx);
		Assert.Equal (0, ry);
   ...
   └──────────────────┘", output);
		...
		Assert.Equal (0, col);
		Assert.Equal (0, row);
		Assert.Null (View.FindDeepestView (top, -4, -3, out var rx, out var ry));
		Assert.Equal (0, rx);
		Assert.Equal (0, ry);
