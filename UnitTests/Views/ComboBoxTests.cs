...
public class ComboBoxTests
{
    ...
    public void EnsureKeyEventsDoNotCauseExceptions ()
    {
        ...
        {
            Assert.Null (Record.Exception (() => comboBox.NewKeyDownEvent (new (new (key)))));
        }
    }
    ...
    public void Expanded_Collapsed_Events ()
    {
        ...
        Assert.True (cb.NewKeyDownEvent (new (Key.F4)));
        Assert.NotNull (cb.Source);
    ...
    public void HideDropdownListOnClick_False_OpenSelectedItem_With_Mouse_And_Key_CursorDown_And_Esc ()
    {
        ...
        Assert.True (cb.Subviews [1].NewKeyDownEvent (new (Key.CursorDown)));
        Assert.Equal ("", selected);
        Assert.True (cb.IsShow);
        Assert.Equal (1, cb.SelectedItem);
        Assert.Equal ("Two", cb.Text);

        Assert.True (cb.Subviews [1].NewKeyDownEvent (new (Key.Enter)));
        ...
        Assert.True (cb.NewKeyDownEvent (new (Key.Esc)));
        Assert.Equal ("Two", selected);
        Assert.False (cb.IsShow);
        Assert.Equal (1, cb.SelectedItem);
        Assert.Equal ("", cb.Text);
...
000002",
        ...
        Assert.True (cb.Subviews [1].NewKeyDownEvent (new (Key.CursorUp)));
        Assert.Equal ("Three", selected);
        Assert.True (cb.IsShow);
        Assert.Equal (2, cb.SelectedItem);
        Assert.Equal ("Three", cb.Text);
...
111112",
    ...
    public void KeyBindings_Command ()
    {
        ...
        Assert.True (cb.NewKeyDownEvent (new (Key.Enter)));
        Assert.False (opened);
        cb.Text = "Tw";
        Assert.True (cb.NewKeyDownEvent (new (Key.Enter)));
        Assert.True (opened);
        ...
        Assert.False (cb.IsShow);
        cb.SetSource (null);
        Assert.False (cb.NewKeyDownEvent (new (Key.Enter)));
        Assert.True (cb.NewKeyDownEvent (new (Key.F4))); // with no source also expand empty
        Assert.True (cb.IsShow);
        Assert.Equal (-1, cb.SelectedItem);
        cb.SetSource (source);
        cb.Text = "";
        Assert.True (cb.NewKeyDownEvent (new (Key.F4))); // collapse
        Assert.False (cb.IsShow);
        Assert.True (cb.NewKeyDownEvent (new (Key.F4))); // expand
        Assert.True (cb.IsShow);
        cb.Collapse ();
        Assert.False (cb.IsShow);
        Assert.True (cb.HasFocus);
        Assert.True (cb.NewKeyDownEvent (new (Key.CursorDown))); // losing focus
        Assert.False (cb.IsShow);
        ...
        Assert.Equal (0, cb.SelectedItem);
        Assert.Equal ("One", cb.Text);
        Assert.True (cb.NewKeyDownEvent (new (Key.CursorDown)));
        Assert.True (cb.IsShow);
        Assert.Equal (1, cb.SelectedItem);
        Assert.Equal ("Two", cb.Text);
        Assert.True (cb.NewKeyDownEvent (new (Key.CursorDown)));
        ...
        Assert.Equal (2, cb.SelectedItem);
        Assert.Equal ("Three", cb.Text);
        Assert.True (cb.NewKeyDownEvent (new (Key.CursorUp)));
        Assert.True (cb.IsShow);
        Assert.Equal (1, cb.SelectedItem);
        Assert.Equal ("Two", cb.Text);
        Assert.True (cb.NewKeyDownEvent (new (Key.CursorUp)));
...
",
        ...
        Assert.True (cb.NewKeyDownEvent (new (Key.PageDown)));
        Assert.True (cb.IsShow);
        Assert.Equal (1, cb.SelectedItem);
        Assert.Equal ("Two", cb.Text);
        Application.Begin (Application.Top);
...
",
        ...
        Assert.True (cb.NewKeyDownEvent (new (Key.PageUp)));
        Assert.True (cb.IsShow);
        Assert.Equal (1, cb.SelectedItem);
        Assert.Equal ("Two", cb.Text);
        Assert.True (cb.NewKeyDownEvent (new (Key.PageUp)));
        ...
        Assert.Equal (0, cb.SelectedItem);
        Assert.Equal ("One", cb.Text);
        Assert.True (cb.NewKeyDownEvent (new (Key.End)));
        Assert.False (cb.IsShow);
        Assert.Equal (0, cb.SelectedItem);
        Assert.Equal ("One", cb.Text);
        Assert.True (cb.NewKeyDownEvent (new (Key.Home)));
        Assert.False (cb.IsShow);
        Assert.Equal (0, cb.SelectedItem);
        Assert.Equal ("One", cb.Text);
        Assert.True (cb.NewKeyDownEvent (new (Key.F4)));
        ...
        Assert.Equal (-1, cb.SelectedItem);
        Assert.Equal ("One", cb.Text);
        Assert.True (cb.NewKeyDownEvent (new (Key.U.WithCtrl)));
        Assert.True (cb.HasFocus);
        Assert.True (cb.IsShow);
        Assert.Equal (-1, cb.SelectedItem);
        Assert.Equal ("", cb.Text);
