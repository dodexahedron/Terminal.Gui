...
public class TableViewTests {
	...
	public void PageDown_ExcludesHeaders ()
	{
		...
		tableView.NewKeyDownEvent (new Key (KeyCode.PageDown));
...
";
	...
	public void TableView_Activate ()
	{
		...
		tv.NewKeyDownEvent (new Key (KeyCode.Z));
		Assert.Equal ("R0C0", activatedValue);
	...
	public void TableViewMultiSelect_CannotFallOffLeft ()
	{
		...
		tv.SelectedColumn = 1;
		tv.SelectedRow = 1;
		tv.NewKeyDownEvent (new Key (KeyCode.CursorLeft | KeyCode.ShiftMask));
	...
	public void TableViewMultiSelect_CannotFallOffRight ()
	{
		...
		tv.SelectedColumn = 0;
		tv.SelectedRow = 1;
		tv.NewKeyDownEvent (new Key (KeyCode.CursorRight | KeyCode.ShiftMask));
	...
	public void TableViewMultiSelect_CannotFallOffTop ()
	{
		...
		tv.SelectedRow = 1;
		tv.NewKeyDownEvent (new Key (KeyCode.CursorLeft | KeyCode.ShiftMask));
		tv.NewKeyDownEvent (new Key (KeyCode.CursorUp | KeyCode.ShiftMask));
...
";
		...
		TestHelpers.AssertDriverAttributesAre (expectedColors, Application.Driver, tv.ColorScheme.Normal, focused ? tv.ColorScheme.Focus : tv.ColorScheme.HotNormal);
...
";
		...
		TestHelpers.AssertDriverAttributesAre (expectedColors, Application.Driver, tv.ColorScheme.Normal, focused ? invertFocus : invertHotNormal);
...
";
		...
		TestHelpers.AssertDriverAttributesAre (expectedColors, Application.Driver, tv.ColorScheme.Normal, focused ? rowHighlight.Focus : rowHighlight.HotNormal, rowHighlight.Normal);
...
";
		...
		TestHelpers.AssertDriverAttributesAre (expectedColors, Application.Driver, tv.ColorScheme.Normal, focused ? tv.ColorScheme.Focus : tv.ColorScheme.HotNormal, cellHighlight.Normal);
...
";
	...
	TableView SetUpMiniTable () => SetUpMiniTable (out _);

	TableView SetUpMiniTable (out DataTable dt)
	{
...
│4│5│6│";
	...
	TableView GetABCDEFTableView (out DataTable dt)
	{
		var tableView = new TableView ();
		tableView.BeginInit ();
		tableView.EndInit ();
...
│☐│1│2│";
	...
	public void TestTableViewCheckboxes_SelectAllToggle ()
	{
		...
		//toggle all cells
		tv.NewKeyDownEvent (new Key (KeyCode.A | KeyCode.CtrlMask));
		tv.NewKeyDownEvent (new Key (KeyCode.Space));
...
│zoo  │o                                         │";
	...
	TableView GetTwoRowSixColumnTable () => GetTwoRowSixColumnTable (out _);

	TableView GetTwoRowSixColumnTable (out DataTable dt)
	{
		var tableView = new TableView ();
		tableView.ColorScheme = Colors.ColorSchemes ["TopLevel"];
	...
	TableView GetPetTable (out EnumerableTableSource<PickablePet> source)
	{
		var tv = new TableView ();
		tv.ColorScheme = Colors.ColorSchemes ["TopLevel"];
