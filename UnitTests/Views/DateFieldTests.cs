...
public class DateFieldTests
{
    ...
    public void Copy_Paste ()
    {
        ...
        // Select all text
        Assert.True (df2.NewKeyDownEvent (new (Key.End.WithShift)));
        Assert.Equal (1, df2.SelectedStart);
        Assert.Equal (10, df2.SelectedLength);
        Assert.Equal (11, df2.CursorPosition);

        // Copy from df2
        Assert.True (df2.NewKeyDownEvent (new (Key.C.WithCtrl)));

        // Paste into df1
        Assert.True (df1.NewKeyDownEvent (new (Key.V.WithCtrl)));
    ...
    public void CursorPosition_Min_Is_Always_One_Max_Is_Always_Max_Format_After_Selection ()
    {
        ...
        // Start selection
        Assert.True (df.NewKeyDownEvent (new (Key.CursorLeft.WithShift)));
        Assert.Equal (1, df.SelectedStart);
        Assert.Equal (1, df.SelectedLength);
        Assert.Equal (0, df.CursorPosition);

        // Without selection
        Assert.True (df.NewKeyDownEvent (new (Key.CursorLeft)));
        Assert.Equal (-1, df.SelectedStart);
        Assert.Equal (0, df.SelectedLength);
        Assert.Equal (1, df.CursorPosition);
        df.CursorPosition = 10;
        Assert.True (df.NewKeyDownEvent (new (Key.CursorRight.WithShift)));
        Assert.Equal (10, df.SelectedStart);
        Assert.Equal (1, df.SelectedLength);
        Assert.Equal (11, df.CursorPosition);
        Assert.True (df.NewKeyDownEvent (new (Key.CursorRight)));
        Assert.Equal (-1, df.SelectedStart);
        Assert.Equal (0, df.SelectedLength);
        Assert.Equal (10, df.CursorPosition);
    }
    ...
    public void KeyBindings_Command ()
    {
        ...
        Assert.True (df.NewKeyDownEvent (new (Key.Delete)));
        Assert.Equal (" 12/12/1971", df.Text);
        df.ReadOnly = false;
        Assert.True (df.NewKeyDownEvent (new (Key.D.WithCtrl)));
        ...
        Assert.Equal (" 02/12/1971", df.Text);
        df.ReadOnly = false;
        Assert.True (df.NewKeyDownEvent (new (Key.Backspace)));
        Assert.Equal (" 02/02/1971", df.Text);
        Assert.True (df.NewKeyDownEvent (new (Key.Home)));
        Assert.Equal (1, df.CursorPosition);
        Assert.True (df.NewKeyDownEvent (new (Key.End)));
        Assert.Equal (10, df.CursorPosition);
        Assert.True (df.NewKeyDownEvent (new (Key.A.WithCtrl)));
        Assert.Equal (1, df.CursorPosition);
        Assert.True (df.NewKeyDownEvent (new (Key.E.WithCtrl)));
        Assert.Equal (10, df.CursorPosition);
        Assert.True (df.NewKeyDownEvent (new (Key.CursorLeft)));
        ...
        Assert.False (df.NewKeyDownEvent (new (Key.A)));
        df.ReadOnly = true;
        df.CursorPosition = 1;
        Assert.True (df.NewKeyDownEvent (new (Key.D1)));
        Assert.Equal (" 02/02/1971", df.Text);
        df.ReadOnly = false;
        Assert.True (df.NewKeyDownEvent (new (Key.D1)));
        ...
        Assert.True (df.NewKeyDownEvent (new (Key.D.WithAlt)));
    ...
    public void Typing_With_Selection_Normalize_Format ()
    {
        ...
        Assert.True (df.NewKeyDownEvent (new (Key.D3)));
