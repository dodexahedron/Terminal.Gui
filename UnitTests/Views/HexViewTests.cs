...
public class HexViewTests
{
    ...
    public void AllowEdits_Edits_ApplyEdits ()
    {
        ...
        hv.AllowEdits = false;
        Assert.True (hv.NewKeyDownEvent (new (Key.Home)));
        Assert.False (hv.NewKeyDownEvent (new (Key.A)));
        Assert.Empty (hv.Edits);
        Assert.Equal (126, hv.Source.Length);

        hv.AllowEdits = true;
        Assert.True (hv.NewKeyDownEvent (new (Key.D4)));
        Assert.True (hv.NewKeyDownEvent (new (Key.D1)));
        ...
        // Appends byte
        Assert.True (hv.NewKeyDownEvent (new (Key.End)));
        Assert.True (hv.NewKeyDownEvent (new (Key.D4)));
        Assert.True (hv.NewKeyDownEvent (new (Key.D2)));
    ...
    public void ApplyEdits_With_Argument ()
    {
        ...
        Assert.True (hv.NewKeyDownEvent (new (Key.D5)));
        Assert.True (hv.NewKeyDownEvent (new (Key.D4)));
    ...
    public void CursorPosition_Encoding_Default ()
    {
        ...
        Assert.True (hv.NewKeyDownEvent (new (Key.Enter)));
        Assert.True (hv.NewKeyDownEvent (new (Key.CursorRight.WithCtrl)));
        Assert.Equal (hv.CursorPosition.X, hv.BytesPerLine);
        Assert.True (hv.NewKeyDownEvent (new (Key.Home)));

        Assert.True (hv.NewKeyDownEvent (new (Key.CursorRight)));
        Assert.Equal (new Point (2, 1), hv.CursorPosition);

        Assert.True (hv.NewKeyDownEvent (new (Key.CursorDown)));
    ...
    public void DisplayStart_Source ()
    {
        ...
        Assert.True (hv.NewKeyDownEvent (new (Key.PageDown)));
    ...
    public void Edited_Event ()
    {
        ...
        Assert.True (hv.NewKeyDownEvent (new (Key.D4)));
        Assert.True (hv.NewKeyDownEvent (new (Key.D6)));
    ...
    public void KeyBindings_Command ()
    {
        ...
        Assert.True (hv.NewKeyDownEvent (new (Key.CursorLeft)));
        Assert.Equal (1, hv.Position);

        Assert.True (hv.NewKeyDownEvent (new (Key.CursorDown)));
        Assert.Equal (5, hv.Position);

        Assert.True (hv.NewKeyDownEvent (new (Key.CursorUp)));
        Assert.Equal (1, hv.Position);

        Assert.True (hv.NewKeyDownEvent (new (Key.V.WithCtrl)));
        ...
        Assert.True (hv.NewKeyDownEvent (new (Key.PageUp)));
        Assert.Equal (1, hv.Position);
        ...
        Assert.True (hv.NewKeyDownEvent (new (Key.CursorLeft.WithCtrl)));
        Assert.Equal (1, hv.Position);

        Assert.True (hv.NewKeyDownEvent (new (Key.CursorDown.WithCtrl)));
        Assert.Equal (37, hv.Position);

        Assert.True (hv.NewKeyDownEvent (new (Key.CursorUp.WithCtrl)));
        Assert.Equal (1, hv.Position);
    }
    ...
    public void PositionChanged_Event ()
    {
        ...
        Assert.True (hv.NewKeyDownEvent (new (Key.CursorRight))); // left side must press twice
        Assert.True (hv.NewKeyDownEvent (new (Key.CursorRight)));
        Assert.True (hv.NewKeyDownEvent (new (Key.CursorDown)));
