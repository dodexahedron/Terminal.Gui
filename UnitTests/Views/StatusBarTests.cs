...
public class StatusBarTests {
	...
	public void StatusBar_Constructor_Default ()
	{
		...
		driver.GetCursorVisibility (out var cv);
	...
	{
		var msg = "";
		var sb = new StatusBar (new StatusItem [] { new (Application.QuitKey, $"{Application.QuitKey} to Quit", () => msg = "Quiting...") });
		Application.Top.Add (sb);
		...
		Application.Iteration += (s, a) => {
			if (iteration == 0) {
				Assert.Equal ("", msg);
				sb.NewKeyDownEvent (new Key (KeyCode.CtrlMask | KeyCode.Q));
			} else if (iteration == 1) {
...
";
	...
	public void CanExecute_ProcessHotKey ()
	{
		...
		var statusBar = new StatusBar (new StatusItem [] {
			new (KeyCode.CtrlMask | KeyCode.N, "~^N~ New", New, CanExecuteNew),
			new (KeyCode.CtrlMask | KeyCode.C, "~^C~ Close", Close, CanExecuteClose)
		});
		...
		Assert.True (top.NewKeyDownEvent (new Key (KeyCode.N | KeyCode.CtrlMask)));
