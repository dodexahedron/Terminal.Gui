...
public class TileViewNesting : Scenario {
	...
	void SetupTileView ()
	{
		...
		var titles = cbTitles.Checked;
		var border = cbBorder.Checked;
		var startHorizontal = cbHorizontal.Checked;
	...
	View CreateContentControl (int number) => (bool)cbUseLabels.Checked ?
		CreateLabelView (number) :
		CreateTextView (number);

	View CreateLabelView (int number) => new Label {
		Width = Dim.Fill (),
		Height = 1,
		AutoSize = false,
	...
	View CreateTextView (int number) => new TextView {
		Width = Dim.Fill (),
		Height = Dim.Fill (),
		Text = number.ToString ().Repeat (1000),
		AllowsTab = false
