...
public class MultiColouredTable : Scenario {
	...
	void EditCurrentCell (object sender, CellActivatedEventArgs e)
	{
		...
		if (GetText ("Enter new value", e.Table.ColumnNames [e.Col], oldValue, out var newText)) {
			try {
	...
	class TableViewColors : TableView {
		...
		{
			var unicorns = render.IndexOf ("unicorns", StringComparison.CurrentCultureIgnoreCase);
			var rainbows = render.IndexOf ("rainbows", StringComparison.CurrentCultureIgnoreCase);
