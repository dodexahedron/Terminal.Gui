...
public class TableEditor : Scenario {
	...
	readonly HashSet<FileSystemInfo> _checkedFileSystemInfos = new ();
	MenuItem _miAlternatingColors;
	MenuItem _miAlwaysShowHeaders;
	MenuItem _miAlwaysUseNormalColorForVerticalCellLines;
	MenuItem _miBottomline;
	MenuItem _miCellLines;
	MenuItem _miCheckboxes;
	MenuItem _miCursor;
	MenuItem _miExpandLastColumn;
	MenuItem _miFullRowSelect;
	MenuItem _miHeaderMidline;
	MenuItem _miHeaderOverline;
	MenuItem _miHeaderUnderline;
	MenuItem _miRadioboxes;

	MenuItem _miShowHeaders;
	MenuItem _miShowHorizontalScrollIndicators;
	MenuItem _miSmoothScrolling;
	ColorScheme alternatingColorScheme;
	DataTable currentTable;

	readonly List<UnicodeRange> Ranges = new() {
		new (0x0000, 0x001F, "ASCII Control Characters"),
		new (0x0080, 0x009F, "C0 Control Characters"),
		new (0x1100, 0x11ff, "Hangul Jamo"), // This is where wide chars tend to start
		...
		new (0x2500, 0x25ff, "Box Drawing & Geometric Shapes"),
		...
		new ((uint)(CharMap.MaxCodePoint - 16), (uint)CharMap.MaxCodePoint, "End"),
	...
	public override void Setup ()
	{
		...
		var statusBar = new StatusBar (new StatusItem [] {
			new (KeyCode.F2, "~F2~ OpenExample", () => OpenExample (true)),
			new (KeyCode.F3, "~F3~ CloseExample", () => CloseExample ()),
			new (KeyCode.F4, "~F4~ OpenSimple", () => OpenSimple (true)),
		...
		tableView.MouseClick += (s, e) => {
			...
			tableView.ScreenToCell (e.MouseEvent.X, e.MouseEvent.Y, out var clickedCol);
	...
	void SortColumn (int clickedCol, string sort, bool isAsc)
	{
		if (currentTable == null) {
			return;
		}
	...
	string TrimArrows (string columnName) => columnName.TrimEnd ((char)CM.Glyphs.UpArrow.Value, (char)CM.Glyphs.DownArrow.Value);

	string StripArrows (string columnName) => columnName.Replace ($"{CM.Glyphs.DownArrow}", "").Replace ($"{CM.Glyphs.UpArrow}", "");
	...
	void ShowHeaderContextMenu (int clickedCol, MouseEventEventArgs e)
	{
		...
		var contextMenu = new ContextMenu (e.MouseEvent.X + 1, e.MouseEvent.Y + 1,
			new MenuBarItem (new MenuItem [] {
				new ($"Hide {TrimArrows (colName)}", "", () => HideColumn (clickedCol)),
				new ($"Sort {StripArrows (sort)}", "", () => SortColumn (clickedCol, sort, isAsc))
			})
		);
	...
	void SetMinAcceptableWidthToOne ()
	{
		for (var i = 0; i < tableView.Table.Columns; i++) {
			var style = tableView.Style.GetOrCreateColumnStyle (i);
			style.MinAcceptableWidth = 1;
		}
	}

	void SetMinAcceptableWidth ()
	{
		var col = GetColumn ();
	...
	void RunColumnWidthDialog (int? col, string prompt, Action<ColumnStyle, int> setter, Func<ColumnStyle, int> getter)
	{
		if (col == null) {
			return;
		}
	...
	void ToggleHorizontalScrollIndicators ()
	{
	...
	void ToggleCheckboxes (bool radio)
	{
	...
	void CheckOrUncheckFile (FileSystemInfo info, bool check)
	{
		if (check) {
	...
	void OpenExample (bool big)
	{
		SetTable (BuildDemoDataTable (big ? 30 : 5, big ? 1000 : 5));
		SetDemoTableStyles ();
	}

	void SetTable (DataTable dataTable) => tableView.Table = new DataTableSource (currentTable = dataTable);
	...
	IEnumerable<FileSystemInfo> GetChildren (FileSystemInfo arg)
	{
		try {
	...
	void OpenTreeExample ()
	{
		...
		var source = new TreeTableSource<FileSystemInfo> (tableView, "Name", tree, new Dictionary<string, Func<FileSystemInfo, object>> {
	...
	string GetHumanReadableFileSize (FileSystemInfo fsi)
	{
	...
	string GetHumanReadableFileSize (long value, CultureInfo culture)
	{
	...
	void OpenSimple (bool big) => SetTable (BuildSimpleDataTable (big ? 30 : 5, big ? 1000 : 5));
	...
	void EditCurrentCell (object sender, CellActivatedEventArgs e)
	{
		...
		if (okPressed) {
			...
			try {
				currentTable.Rows [e.Row] [tableCol] = string.IsNullOrWhiteSpace (tf.Text) ? DBNull.Value : tf.Text;
			} catch (Exception ex) {
	...
	bool HasCheckboxes () => tableView.Table is CheckBoxTableSourceWrapperBase;

	string GetUnicodeCategory (uint u) => Ranges.FirstOrDefault (r => u >= r.Start && u <= r.End)?.Category ?? "Unknown";
	...
	/// <summary>
	/// Builds a simple table in which cell values contents are the index of the cell.  This helps testing that scrolling etc
	/// is working correctly and not skipping out any rows/columns when paging
	/// </summary>
	/// <param name="cols"></param>
	/// <param name="rows"></param>
	/// <returns></returns>
