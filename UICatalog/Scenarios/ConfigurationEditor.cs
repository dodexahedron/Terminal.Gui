...
public class ConfigurationEditor : Scenario {
	...
	static ColorScheme _editorColorScheme = new() {
	...
	static Action _editorColorSchemeChanged;
	...
	public override void Setup ()
	{
		...
		var statusBar = new StatusBar (new [] {
			new (Application.QuitKey, $"{Application.QuitKey} Quit", () => Quit ()),
	...
	void Quit ()
	{
		foreach (var tile in _tileView.Tiles) {
			...
			if (editor.IsDirty) {
				var result = MessageBox.Query ("Save Changes", $"Save changes to {editor.FileInfo.FullName}", "Yes", "No", "Cancel");
	...
	class ConfigTextView : TextView {

		internal ConfigTextView () => ContentsChanged += (s, obj) => {
			if (IsDirty) {
