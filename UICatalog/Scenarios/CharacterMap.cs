...
public class CharacterMap : Scenario {
	...
	public override void Setup ()
	{
		...
		_categoryList.MouseClick += (s, e) => {
			_categoryList.ScreenToCell (e.MouseEvent.X, e.MouseEvent.Y, out var clickedCol);
		...
		_categoryList.Style.ColumnStyles.Add (0,
	...
	void _categoryList_Initialized (object sender, EventArgs e) =>
	...
	void JumpEdit_TextChanged (object sender, TextChangedEventArgs e)
	{
		...
		if (jumpEdit.Text.StartsWith ("U+", StringComparison.OrdinalIgnoreCase) ||
		    jumpEdit.Text.StartsWith ("\\u")) {
			try {
		...
		} else if (jumpEdit.Text.StartsWith ("0", StringComparison.OrdinalIgnoreCase) ||
			   jumpEdit.Text.StartsWith ("\\u")) {
			try {
...
class CharMap : ScrollView {
		...
		set {
			...
			if (IsInitialized) {
				...
				if (row + ContentOffset.Y < 0) {
					...
					// Moving down.
					ContentOffset = new Point (ContentOffset.X,
						Math.Min (row, row - height + _rowHeight));
				}

				var width = (Bounds.Width / COLUMN_WIDTH * COLUMN_WIDTH) -
					    (ShowVerticalScrollIndicator ? RowLabelWidth + 1 : RowLabelWidth);
				if (col + ContentOffset.X < 0) {
					// Moving left.
					ContentOffset = new Point (col, ContentOffset.Y);
				} else if (col + ContentOffset.X >= width) {
					// Moving right.
					ContentOffset = new Point (Math.Min (col, col - width + COLUMN_WIDTH),
						ContentOffset.Y);
				}
			}
	...
	public Point Cursor {
		get {
			...
			var col = (SelectedCodePoint % 16 * COLUMN_WIDTH) + ContentOffset.X + RowLabelWidth +
				  1; // + 1 for padding
	...
	public override void OnDrawContentComplete (Rect contentArea)
	{
		...
		if (ShowHorizontalScrollIndicator) {
			...
			Driver.Clip = new Rect (Driver.Clip.Location,
		...
		for (var hexDigit = 0; hexDigit < 16; hexDigit++) {
			...
			if (x > RowLabelWidth - 2) {
				...
				Driver.SetAttribute (GetHotNormalColor ());
				Driver.AddStr (" ");
				Driver.SetAttribute (HasFocus && cursorCol + ContentOffset.X + RowLabelWidth == x
		...
		for (var y = 1; y < Bounds.Height; y++) {
			...
			for (var col = 0; col < 16; col++) {
				...
				Move (x, y);
				if (cursorRow + ContentOffset.Y + 1 == y &&
				    cursorCol + ContentOffset.X + firstColumnX + 1 == x && !HasFocus) {
			...
			Move (0, y);
			Driver.SetAttribute (HasFocus && cursorRow + ContentOffset.Y + 1 == y
				? ColorScheme.HotFocus
		...
		if (me.Flags == _contextMenu.MouseFlags) {
			...
			_contextMenu = new ContextMenu {
				...
				MenuItems = new MenuBarItem (new MenuItem [] {
					...
					new("Copy Code _Point", "", CopyCodePoint, null, null,
						(KeyCode)Key.C.WithCtrl.WithShift)
				})
			};
			_contextMenu.Show ();
		}
	...
	void ShowDetails ()
	{
		...
		if (!string.IsNullOrEmpty (decResponse)) {
			...
			MessageBox.ErrorQuery ("Code Point API",
...
public class UcdApiClient {
	...
	{
		var response = await _httpClient.GetAsync ($"{BaseUrl}codepoint/hex/{hex}");
	...
	{
		var response = await _httpClient.GetAsync ($"{BaseUrl}codepoint/dec/{dec}");
		response.EnsureSuccessStatusCode ();
		return await response.Content.ReadAsStringAsync ();
	}
	...
	{
		var response = await _httpClient.GetAsync ($"{BaseUrl}chars/{Uri.EscapeDataString (chars)}");
		response.EnsureSuccessStatusCode ();
		return await response.Content.ReadAsStringAsync ();
	}
	...
	{
		var response = await _httpClient.GetAsync ($"{BaseUrl}chars/{Uri.EscapeDataString (chars)}/name");
		response.EnsureSuccessStatusCode ();
		return await response.Content.ReadAsStringAsync ();
	}
}
...
class UnicodeRange {
	...
	{
		var ranges = from r in typeof (UnicodeRanges).GetProperties (BindingFlags.Static | BindingFlags.Public)
