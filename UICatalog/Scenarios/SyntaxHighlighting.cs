...
public class SyntaxHighlighting : Scenario {
	...
	readonly HashSet<string> keywords = new (StringComparer.CurrentCultureIgnoreCase) {
	...
	ColorScheme magenta;
	MenuItem miWrap;
	readonly string path = "RuneCells.rce";
	...
	void HighlightTextBasedOnKeywords ()
	{
		...
		var keywordRegexes = keywords.Select (k => new Regex ($@"\b{k}\b", RegexOptions.IgnoreCase)).ToArray ();
		var keywordMatches = keywordRegexes.SelectMany (r => r.Matches (textView.Text)).ToArray ();
	...
	/// </para>
	/// <para>
	/// If there are public properties/variables that you do not want written to the file, decorate them with the [JsonIgnore]
	/// attribute.
	/// </para>
	/// </summary>
