...
class KeyBindingsDialog : Dialog {
	...
	readonly ListView commandsListView;
	...
	void CommandsListView_SelectedItemChanged (object sender, ListViewItemEventArgs obj) => SetTextBoxToShowBinding ((Command)obj.Value);
	...
	class ViewTracker {
		...
		Dictionary<Command, KeyCode> keybindings;

		/// <summary>
		/// All views seen so far and a bool to indicate if we have applied keybindings to them
		/// </summary>
		readonly Dictionary<View, bool> knownViews = new ();

		readonly object lockKnownViews = new ();
