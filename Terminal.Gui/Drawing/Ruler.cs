...
public class Ruler {
	...
	public void Draw (Point location, int start = 0)
	{
		...
		if (Orientation == Orientation.Horizontal) {
			var hrule = _hTemplate.Repeat ((int)Math.Ceiling (Length + 2 / (double)_hTemplate.Length)) [start..(Length + start)];
			// Top
			Application.Driver.Move (location.X, location.Y);
			Application.Driver.AddStr (hrule);

		} else {
			var vrule = _vTemplate.Repeat ((int)Math.Ceiling ((Length + 2) / (double)_vTemplate.Length)) [start..(Length + start)];
