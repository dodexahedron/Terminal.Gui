...
public class StraightLine {
	...
	IntersectionDefinition? IntersectsHorizontally (int x, int y)
	{
		if (Start.Y != y) {
			return null;
		}
	...
	IntersectionDefinition? IntersectsVertically (int x, int y)
	{
		if (Start.X != x) {
			return null;
		}
	...
	IntersectionType GetTypeByLength (IntersectionType typeWhenNegative, IntersectionType typeWhenZero, IntersectionType typeWhenPositive)
	{
		if (Length == 0) {
			return typeWhenZero;
		}
	...
	bool StartsAt (int x, int y) => Start.X == x && Start.Y == y;
