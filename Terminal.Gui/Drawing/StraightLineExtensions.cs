...
public static class StraightLineExtensions {
	...
	public static IEnumerable<StraightLine> Exclude (this IEnumerable<StraightLine> collection, Point start, int length, Orientation orientation)
	{
		...
		foreach (var l in collection) {
			...
			var econstPoint = orientation == Orientation.Horizontal ? start.Y : start.X;
			var lconstPoint = l.Orientation == Orientation.Horizontal ? l.Start.Y : l.Start.X;

			// For the varying axis what is the max/mins
			// i.e. points on horizontal lines vary by x, vertical lines vary by y
			var eDiffMin = GetLineStartOnDiffAxis (start, length, orientation);
			var eDiffMax = GetLineEndOnDiffAxis (start, length, orientation);
			var lDiffMin = GetLineStartOnDiffAxis (l.Start, l.Length, l.Orientation);
			var lDiffMax = GetLineEndOnDiffAxis (l.Start, l.Length, l.Orientation);
	...
	static int GetLineStartOnDiffAxis (Point start, int length, Orientation orientation)
	{
		if (length == 0) {
	...
	static int GetLineEndOnDiffAxis (Point start, int length, Orientation orientation)
	{
		if (length == 0) {
			throw new ArgumentException ("0 length lines are not supported", nameof (length));
		}
	...
	static StraightLine CreateLineFromDiff (StraightLine l, int from, int length)
	{
		var start = new Point (
