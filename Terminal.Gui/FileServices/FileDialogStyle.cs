...
public class FileDialogStyle {
	...
	/// <see cref="OpenMode"/> is <see cref="OpenMode.Directory"/> and <see cref="FileDialog.MustExist"/> is
	/// <see langword="true"/>.
	/// </summary>
	...
	Dictionary<IDirectoryInfo, string> DefaultTreeRootGetter ()
	{
		var roots = new Dictionary<IDirectoryInfo, string> ();
		try {
			foreach (var d in GetLogicalDrives ()) {
		...
		try {
			foreach (var special in Enum.GetValues (typeof (SpecialFolder)).Cast<SpecialFolder> ()) {
				try {
					var path = GetFolderPath (special);
