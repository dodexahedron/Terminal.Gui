...
public sealed partial record Key : IEqualityOperators<Key,Key,bool>
{
    ...
    /// <remarks></remarks>
    // TODO: Refactor this.
    // Ideally, this should either call a constructor or can be called by one, but not both.
    // Perhaps the validation part of it can be pulled out and used by each to de-couple them.
    public static bool TryParse (string? text, [NotNullWhen (true)] out Key? key)
    {
        ...
        string partNotFound = parts.FirstOrDefault (static p => !string.IsNullOrEmpty (p), string.Empty);
