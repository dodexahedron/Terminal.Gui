...
public struct Rect {
	...
	public static Rect FromLTRB (int left, int top,
		int right, int bottom) => new (left, top, right - left,
		bottom - top);
	...
	public static Rect Union (Rect a, Rect b)
	{
		...
		var x2 = Math.Max (a.X + Math.Abs (a.Width), b.X + Math.Abs (b.Width));
		var y1 = Math.Min (a.Y, b.Y);
		var y2 = Math.Max (a.Y + Math.Abs (a.Height), b.Y + Math.Abs (b.Height));
		return new Rect (x1, y1, x2 - x1, y2 - y1);
	}
	...
	/// </remarks>
	public static bool operator == (Rect left, Rect right) => left.Location == right.Location &&
	...
	/// </remarks>
	public bool Contains (int x, int y) => x >= Left && x < Right &&
	                                       y >= Top && y < Bottom;
	...
	/// Rectangle.
	/// </remarks>
	public bool Contains (Rect rect) => rect == Intersect (this, rect);
	...
	/// </remarks>
	public bool IntersectsWith (Rect rect) => !(Left >= rect.Right || Right <= rect.Left ||
	                                            Top >= rect.Bottom || Bottom <= rect.Top);

	bool IntersectsWithInclusive (Rect r) => !(Left > r.Right || Right < r.Left ||
