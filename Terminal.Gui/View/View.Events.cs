...
public partial class View
{
    ...
    /// <param name="e">
    ///     Data for the <see cref="PropertyChanged"/> event. If this method is called explicitly (ie not by calling <see cref="SetField{T}"/>), you MUST supply the correct and exact
    ///     name of the property that was changed.
    /// </param>
    /// <remarks>
    ///     Do not hide this method. To perform your own actions before and after raising the event, implement your own method that calls this method when it is ready for the event to be
    ///     raised.
    /// </remarks>
    protected internal void RaisePropertyChanged (View? sender, PropertyChangedEventArgs e) { PropertyChanged?.Invoke (sender, e); }
    ...
    /// <param name="sender">The specific object requesting that the event be raised.</param>
    /// <param name="e">
    ///     Data for the <see cref="PropertyChanging"/> event. If this method is called explicitly (ie not by calling <see cref="SetField{T}"/>), you MUST supply the correct and exact
    ...
    ///     raised.
    /// </remarks>
    protected internal void RaisePropertyChanging (View? sender, PropertyChangingEventArgs e) { PropertyChanging?.Invoke (sender, e); }
    ...
    /// <summary>
    ///     Sets the referenced <paramref name="field"/> to <paramref name="value"/> **BY VALUE** and raises <see cref="PropertyChanged"/>.
    /// </summary>
    /// <typeparam name="T">
    ///     The type of the field and value. The value passed to <paramref name="value"/> can be another type, so long as it is assignable to the exact type of <paramref name="field"/>.
    /// </typeparam>
    /// <param name="field">
    ///     A reference to the field to be changed. Note that this is a ref parameter and may access anything up to the scope of the caller context at time of assignment.
    /// </param>
    /// <param name="value">
    ///     The new value to assign to <paramref name="field"/>. Value types will be copied by the compiler. Reference types will copy only the reference.
    /// </param>
    /// <param name="propertyName">
    ///     The name of the property being changed. Values supplied explicitly will be ignored by the compiler and will be replaced by the unqualified name of the member that called this
    ///     method.
    /// </param>
    /// <returns>
    ...
    ///     Argument passed to the <paramref name="field"/> parameter is a <see langword="ref"/> <typeparamref name="T"/> and must be an assignable field, parameter, or local variable in
    ///     a reachable from the caller's ref-safe context.<br/>
    ///     While you may be able to fool the compiler into letting you _compile_ code that breaks the rules, in certain situations, tht code WILL break at run-time.<br/>
