...
public class TextFormatter {
	...
	/// <summary>
	/// The specifier character for the hot key (e.g. '_'). Set to '\xffff' to disable hot key support for this View instance.
	...
	public void Draw (Rect bounds, Attribute normalColor, Attribute hotColor, Rect containerBounds = default, bool fillRemaining = true, ConsoleDriver driver = null)
	{
		...
		for (var line = lineOffset; line < linesFormated.Count; line++) {
	...
	public static string ClipOrPad (string text, int width)
	{
		...
		if (text.EnumerateRunes ().Sum (c => c.GetColumns ()) < width) {
			...
			var toPad = width - text.EnumerateRunes ().Sum (c => c.GetColumns ());
	...
	/// If <see langword="true"/> trailing spaces at the end of wrapped lines will be preserved.
	/// If <see langword="false"/>, trailing spaces at the end of wrapped lines will be trimmed.
	/// </param>
		...
		TextDirection textDirection = TextDirection.LeftRight_TopBottom)
	{
		...
		if (preserveTrailingSpaces) {
			while ((end = start) < runes.Count) {
				end = GetNextWhiteSpace (start, width, out var incomplete);
				if (end == 0 && incomplete) {
			...
			if (IsHorizontalDirection (textDirection)) {
				...
				while ((end = start + width) < runes.Count) {
					...
					var zeroLength = 0;
					for (var i = end; i < runes.Count - start; i++) {
						var r = runes [i];
						if (r.GetColumns () == 0) {
							zeroLength++;
						} else {
	...
	public static string ClipAndJustify (string text, int width, TextAlignment talign, TextDirection textDirection = TextDirection.LeftRight_TopBottom, int tabWidth = 0) => ClipAndJustify (text, width, talign == TextAlignment.Justified, textDirection, tabWidth);
	...
	/// <param name="wordWrap">
	/// If <see langword="true"/>, the text will be wrapped to new lines no longer than <paramref name="width"/>.
	/// If <see langword="false"/>, forces text to fit a single line. Line breaks are converted to spaces. The text will be
	...
	///         </para>
	/// </remarks>
	public static List<string> Format (string text, int width, TextAlignment talign, bool wordWrap, bool preserveTrailingSpaces = false, int tabWidth = 0, TextDirection textDirection = TextDirection.LeftRight_TopBottom, bool multiLine = false) => Format (text, width, talign == TextAlignment.Justified, wordWrap, preserveTrailingSpaces, tabWidth, textDirection, multiLine);
	...
	{
		var result = Format (text, maxColumns, false, true);
		var max = 0;
		result.ForEach (s => {
			var m = 0;
	...
	/// <see cref="StringExtensions.GetColumns"/>).
	...
	{
		var max = 0;
		for (var i = startIndex == -1 ? 0 : startIndex; i < (length == -1 ? lines.Count : startIndex + length); i++) {
			var runes = lines [i];
			if (runes.Length > 0) {
	...
	public static int GetSumMaxCharWidth (string text, int startIndex = -1, int length = -1, int tabWidth = 0)
	{
		...
		for (var i = startIndex == -1 ? 0 : startIndex; i < (length == -1 ? runes.Length : startIndex + length); i++) {
	...
	static int GetRuneWidth (string str, int tabWidth) => GetRuneWidth (str.EnumerateRunes ().ToList (), tabWidth);

	static int GetRuneWidth (List<Rune> runes, int tabWidth) => runes.Sum (r => GetRuneWidth (r, tabWidth));

	static int GetRuneWidth (Rune rune, int tabWidth)
	{
	...
	/// If <c>true</c> the legacy behavior of identifying the
