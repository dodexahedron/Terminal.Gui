...
public abstract class PopupAutocomplete : AutocompleteBase {
	...
	void Top_Removed (object sender, SuperViewChangedEventArgs e)
	{
		Visible = false;
		ManipulatePopup ();
	}

	void Top_DrawContentComplete (object sender, DrawEventArgs e) => ManipulatePopup ();

	void Top_DrawContent (object sender, DrawEventArgs e)
	{
		if (!closed) {
			ReopenSuggestions ();
		}
	...
	class Popup : View {
		readonly PopupAutocomplete autocomplete;
