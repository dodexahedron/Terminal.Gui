...
/// </remarks>
/// <para>
/// <see cref="ThemeManager"/> is a singleton class. It is created when the first <see cref="ThemeManager"/> property is
/// accessed.
...
public class ThemeManager : IDictionary<string, ThemeScope> {
	...
	static string _theme = string.Empty;
	static ThemeManager () { } // Make sure it's truly lazy
	ThemeManager () { } // Prevent instantiation outside
...
#pragma warning disable 1591
	...
	public bool TryGetValue (string key, out ThemeScope value) => ((IDictionary<string, ThemeScope>)Themes!).TryGetValue (key, out value!);
	...
	public bool Contains (KeyValuePair<string, ThemeScope> item) => ((ICollection<KeyValuePair<string, ThemeScope>>)Themes!).Contains (item);

	public void CopyTo (KeyValuePair<string, ThemeScope> [] array, int arrayIndex) => ((ICollection<KeyValuePair<string, ThemeScope>>)Themes!).CopyTo (array, arrayIndex);

	public bool Remove (KeyValuePair<string, ThemeScope> item) => ((ICollection<KeyValuePair<string, ThemeScope>>)Themes!).Remove (item);

	public IEnumerator<KeyValuePair<string, ThemeScope>> GetEnumerator () => ((IEnumerable<KeyValuePair<string, ThemeScope>>)Themes!).GetEnumerator ();
