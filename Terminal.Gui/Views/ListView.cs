...
public class ListView : View {
	...
	/// </summary>
	/// <param name="source">
	/// An <see cref="IList"/> data source, if the elements are strings or ustrings,
	...
	/// <remarks>
	/// If set to <see langword="true"/>, <see cref="ListView"/> will render items marked items with "[x]", and unmarked items
	/// with "[ ]"
	...
	/// </remarks>
	public Task SetSourceAsync (IList source) => Task.Factory.StartNew (() => {
		if (source == null && (Source == null || !(Source is ListWrapper))) {
			Source = null;
		} else {
			Source = MakeWrapper (source);
	...
	static IListDataSource MakeWrapper (IList source) => new ListWrapper (source);
	...
	void ListView_LayoutStarted (object sender, LayoutEventArgs e) => EnsureSelectedItemVisible ();
