...
class Branch<T> where T : class {
	...
	/// </summary>
	/// <returns></returns>
	public virtual int GetWidth (ConsoleDriver driver) => GetLinePrefix (driver).Sum (r => r.GetColumns ()) +
	...
	public virtual void Draw (ConsoleDriver driver, ColorScheme colorScheme, int y, int availableWidth)
	{
		...
		var isSelected = tree.IsSelected (Model);

		var textColor = isSelected ? tree.HasFocus ? colorScheme.Focus : colorScheme.HotNormal : colorScheme.Normal;
		var symbolColor = tree.Style.HighlightModelTextOnly ? colorScheme.Normal : textColor;

		// Everything on line before the expansion run and branch text
		var prefix = GetLinePrefix (driver).ToArray ();
		var expansion = GetExpandableSymbol (driver);
		var lineBody = tree.AspectGetter (Model) ?? "";
	...
	static RuneCell NewRuneCell (Attribute attr, Rune r) => new() {
		Rune = r,
	...
	/// </summary>
	/// <returns></returns>
	IEnumerable<Branch<T>> GetParentBranches ()
	{
		var cur = Parent;
	...
	{
		var leafSymbol = tree.Style.ShowBranchLines ? Glyphs.HLine : (Rune)' ';
	...
	/// True to also refresh all <see cref="Parent"/>
