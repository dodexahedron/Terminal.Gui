...
public class TileView : View {
	...
	List<Pos> _splitterDistances;
	List<TileViewLineView> _splitterLines;
	...
	bool RecursiveContains (IEnumerable<View> haystack, View needle)
	{
	...
	/// <summary>
	///         <para>
	///         Attempts to update the <see cref="SplitterDistances"/> of line at <paramref name="idx"/>
	...
	///         </para>
	///         <para>
	///         Only absolute values (e.g. 10) and percent values (i.e. <see cref="Pos.Percent(float)"/>)
	...
	public override void OnDrawContent (Rect contentArea)
	{
		...
		if (IsRootTileView ()) {
			...
			foreach (var line in allLines) {
				var isRoot = _splitterLines.Contains (line);
	...
	/// <paramref name="result"/> <see cref="TileView"/>'s first tile.
	/// </remarks>
	...
	/// <returns>
	/// <see langword="true"/> if a <see cref="View"/> was converted to a new nested
	...
	bool IsValidNewSplitterPos (int idx, Pos value, int fullSpace)
	{
		var newSize = value.Anchor (fullSpace);
		var isGettingBigger = newSize > _splitterDistances [idx].Anchor (fullSpace);
		var lastSplitterOrBorder = HasBorder () ? 1 : 0;
		var nextSplitterOrBorder = HasBorder () ? fullSpace - 1 : fullSpace;
		...
		if (idx > 0) {
			var posLeft = _splitterDistances [idx - 1].Anchor (fullSpace);
		...
		if (idx + 1 < _splitterDistances.Count) {
			var posRight = _splitterDistances [idx + 1].Anchor (fullSpace);
	...
	List<TileViewLineView> GetAllLineViewsRecursively (View v)
	{
	...
	List<TileTitleToRender> GetAllTitlesToRenderRecursively (TileView v, int depth = 0)
	{
	...
	/// </summary>
	/// <remarks>
	/// Note that manually adding one <see cref="TileView"/> to another will not result in a parent/child
	...
	Dim GetTileWidthOrHeight (int i, int space, Tile [] visibleTiles, TileViewLineView [] visibleSplitterLines)
	{
		// last tile
	...
	bool HasBorder () => LineStyle != LineStyle.None;
	...
	class TileTitleToRender {
		...
		{
			var spaceDim = Tile.ContentView.Width;
	...
	class TileViewLineView : LineView {
		Pos dragOrignalPos;
		...
		bool FinalisePosition (Pos oldValue, Pos newValue)
		{
		...
		///         </para>
		///         <para>
		///         Effectively turning any <see cref="Pos"/> into a <see cref="Pos.PosFactor"/>
		///         (as if created with <see cref="Pos.Percent(float)"/>)
		///         </para>
		/// </summary>
		...
		/// <returns></returns>
		Pos ConvertToPosFactor (Pos p, int parentLength)
		{
			// calculate position in the 'middle' of the cell at p distance along parentLength
			var position = p.Anchor (parentLength) + 0.5f;
