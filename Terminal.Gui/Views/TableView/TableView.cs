...
public class TableView : View {
	...
	KeyCode cellActivationKey = KeyCode.Enter;
	...
	/// <summary>
	/// When <see cref="MultiSelect"/> is enabled this property contain all rectangles of selected cells.  Rectangles describe
	/// column/rows selected in <see cref="Table"/> (not screen coordinates)
	/// </summary>
	/// <returns></returns>
	...
	/// <summary>
	/// Vertical scroll offset.  The index of the first row in <see cref="Table"/> to display in the first non header line of
	/// the control when rendering the view.
	/// </summary>
	...
	/// <summary>
	/// The maximum number of characters to render in any given column.  This prevents one long column from pushing out all the
	/// others
	/// </summary>
	...
	{
		var heightRequired = Style.ShowHeaders ? 1 : 0;
	...
	void RenderHeaderOverline (int row, int availableWidth, ColumnToRender [] columnsToRender)
	{
	...
	void RenderHeaderMidline (int row, ColumnToRender [] columnsToRender)
	{
	...
	void RenderHeaderUnderline (int row, int availableWidth, ColumnToRender [] columnsToRender)
	{
		/*
		...
		*/
		...
		var moreColumnsToLeft = ColumnOffset > 0;
		...
		var lastColumnIdxRendered = ColumnOffset + columnsToRender.Length - 1;

		// are there more valid indexes?
		var moreColumnsToRight = lastColumnIdxRendered < Table.Columns;
	...
	void RenderBottomLine (int row, int availableWidth, ColumnToRender [] columnsToRender)
	{
	...
	void RenderRow (int row, int rowToRender, ColumnToRender [] columnsToRender)
	{
		...
		for (var i = 0; i < columnsToRender.Length; i++) {
			...
			var isSelectedCell = IsSelected (current.Column, rowToRender);
			...
			var isPrimaryCell = current.Column == selectedColumn && rowToRender == selectedRow;
	...
	void RenderSeparator (int col, int row, bool isHeader)
	{
		if (col < 0) {
			return;
		}
		...
		var symbol = renderLines ? Glyphs.VLine : (Rune)SeparatorSymbol;
	...
	/// <paramref name="availableHorizontalSpace"/> using the alignment specified in <paramref name="colStyle"/> (or left if no
	/// style is defined)
	/// </summary>
	...
	/// <returns></returns>
	string TruncateOrPad (object originalCellValue, string representation, int availableHorizontalSpace, ColumnStyle colStyle)
	{
		...
		if (representation.EnumerateRunes ().Sum (c => c.GetColumns ()) < availableHorizontalSpace) {
			...
			var toPad = availableHorizontalSpace - (representation.EnumerateRunes ().Sum (c => c.GetColumns ()) + 1 /*leave 1 space for cell boundary*/);
	...
	bool CycleToNextTableEntryBeginningWith (Key keyEvent)
	{
		var row = SelectedRow;
		...
		var match = CollectionNavigator.GetNextMatchingItem (row, (char)keyEvent);
	...
	public void SetSelection (int col, int row, bool extendExistingSelection)
	{
		...
		var lookRight = col > selectedColumn;
	...
	void ClearMultiSelectedRegions (bool keepToggledSelections)
	{
	...
	/// <param name="col"></param>
	/// <param name="row"></param>
	void UnionSelection (int col, int row)
	{
	...
	/// <summary>
	/// Moves the <see cref="SelectedRow"/> and <see cref="SelectedColumn"/> by the provided offsets. Optionally starting a box
	/// selection (see <see cref="MultiSelect"/>)
	/// </summary>
	...
	/// <summary>
	/// Returns all cells in any <see cref="MultiSelectedRegions"/> (if <see cref="MultiSelect"/> is enabled) and the selected
	/// cell
	/// </summary>
	/// <returns></returns>
	...
	/// <returns></returns>
	TableSelection CreateTableSelection (int pt1X, int pt1Y, int pt2X, int pt2Y, bool toggle = false)
	{
	...
	/// <param name="y"></param>
	/// <returns></returns>
	TableSelection CreateTableSelection (int x, int y) => CreateTableSelection (x, y, x, y);
	...
	IEnumerable<TableSelection> GetMultiSelectedRegionsContaining (int col, int row)
	{
		if (!MultiSelect) {
	...
	bool HasControlOrAlt (MouseEvent me) => me.Flags.HasFlag (MouseFlags.ButtonAlt) || me.Flags.HasFlag (MouseFlags.ButtonCtrl);
	...
	/// <summary>
	/// Updates the view to reflect changes to <see cref="Table"/> and to (<see cref="ColumnOffset"/> / <see cref="RowOffset"/>
	/// ) etc
	/// </summary>
	...
	/// <summary>
	/// Updates <see cref="SelectedColumn"/>, <see cref="SelectedRow"/> and <see cref="MultiSelectedRegions"/> where they are
	/// outside the bounds of the table (by adjusting them to the nearest existing cell).  Has no effect if <see cref="Table"/>
	/// has not been set.
	/// </summary>
	/// <remarks>
	/// Changes will not be immediately visible in the display until you call <see cref="View.SetNeedsDisplay()"/>
	...
	/// </summary>
	/// <returns></returns>
	bool TableIsNullOrInvisible () => Table == null ||
	                                  Table.Columns <= 0 ||
	...
	/// <see langword="true"/> to look right, <see langword="false"/> to look left.
	/// </param>
	...
	/// </param>
	int GetNearestVisibleColumn (int columnIndex, bool lookRight, bool allowBumpingInOppositeDirection)
	{
	...
	bool TryGetNearestVisibleColumn (int columnIndex, bool lookRight, bool allowBumpingInOppositeDirection, out int idx)
	{
		// if the column index provided is out of bounds
		if (columnIndex < 0 || columnIndex >= table.Columns) {
		...
		var increment = lookRight ? 1 : -1;

		// move in that direction
		for (var i = columnIndex; i >= 0 && i < columnVisibility.Length; i += increment) {
	...
	/// <summary>
	/// Updates scroll offsets to ensure that the selected cell is visible.  Has no effect if <see cref="Table"/> has not been
	/// set.
	/// </summary>
	/// <remarks>
	/// Changes will not be immediately visible in the display until you call <see cref="View.SetNeedsDisplay()"/>
	...
	/// <returns></returns>
	IEnumerable<ColumnToRender> CalculateViewport (Rect bounds, int padding = 1)
	{
		if (TableIsNullOrInvisible ()) {
	...
	/// <summary>
	/// Returns the maximum of the <paramref name="col"/> name and the maximum length of data that will be rendered starting at
	/// <see cref="RowOffset"/> and rendering <paramref name="rowsToRender"/>
	/// </summary>
	/// <param name="col"></param>
	/// <param name="rowsToRender"></param>
	/// <param name="colStyle"></param>
	/// <returns></returns>
	int CalculateMaxCellWidth (int col, int rowsToRender, ColumnStyle colStyle)
	{
		var spaceRequired = table.ColumnNames [col].EnumerateRunes ().Sum (c => c.GetColumns ());
		...
		for (var i = RowOffset; i < RowOffset + rowsToRender && i < Table.Rows; i++) {
	...
	/// <returns></returns>
	string GetRepresentation (object value, ColumnStyle colStyle)
	{
	...
	internal class ColumnToRender {
		...
		/// <summary>
		/// The width that the column should occupy as calculated by <see cref="CalculateViewport(Rect, int)"/>.  Note that this
		/// includes
