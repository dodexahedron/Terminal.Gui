...
public class ListTableSource : ITableSource {
	...
	Rect _lastBounds;
	IList _lastList;
	int _lastMaxCellWidth;
	int _lastMinCellWidth;
	ListColumnStyle _lastStyle;
	...
	void TableView_DrawContent (object sender, DrawEventArgs e)
	{
	...
	/// </summary>
	DataTable CreateTable (int cols = 1)
	{
	...
	int CalculateColumns ()
	{
		...
		var colWidth = CalculateMaxLength ();
		...
		if (Style.Orientation == Orientation.Vertical != Style.ScrollParallel) {
			var f = (float)_tableView.Bounds.Height - _tableView.GetHeaderHeight ();
