...
///         <see cref="WizardStep" />) can host
///         arbitrary <see cref="View" />s, much like a <see cref="Dialog" />. Each step also has a pane for help text.
///         Along the
///         bottom of the Wizard view are customizable buttons enabling the user to navigate forward and backward through
///         the Wizard.
/// </summary>
/// <remarks>
///         The Wizard can be displayed either as a modal (pop-up) <see cref="Window" /> (like <see cref="Dialog" />) or as
///         an embedded <see cref="View" />.
///         By default, <see cref="Wizard.Modal" /> is <c>true</c>. In this case launch the Wizard with
///         <c>Application.Run(wizard)</c>.
...
public class Wizard : Dialog {
	readonly LinkedList<WizardStep> _steps = new ();
	...
	//	}
	//}
	string _wizardTitle = string.Empty;
	...
	///                 </item>
	///         </list>
	///         If a non-Modal Wizard is added to the application after <see cref="Application.Run(Func{Exception, bool})" />
	///         has been called
	...
	void Wizard_Loaded (object sender, EventArgs args) =>
	...
	///         <see cref="OnProcessKeyDown" />
	///         to instead fire the <see cref="Cancelled" /> event when Wizard is being used as a non-modal (see
	///         <see cref="Wizard.Modal" />.
	/// </summary>
	/// <param name="a"></param>
	/// <returns></returns>
	...
	/// </summary>
	/// <returns>
	///         The next step after the current step, if there is one; otherwise returns <c>null</c>, which
	...
	/// </summary>
	/// <returns>
	///         The first step ahead of the current step, if there is one; otherwise returns <c>null</c>, which
	...
	///         The Next/Finish button is always the last button in the array of Buttons passed to the <see cref="Wizard" />
	///         constructor,
	...
	///         the last button in the array of Buttons passed to the <see cref="Wizard" /> constructor, if any. This event is
	///         only
	...
	///         <see cref="StepChangeEventArgs.Cancel" />
	...
	void UpdateButtonsAndTitle ()
	{
		...
		BackButton.SetDesiredVisibility (CurrentStep != GetFirstStep ());
