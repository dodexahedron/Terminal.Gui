...
///         divided horizontally in two. On the left is the
...
///         be shown.
///         If there are no Views added to the WizardStep the <see cref="HelpText" /> (if not empty) will fill the wizard
///         step.
/// </summary>
/// <remarks>
...
public class WizardStep : FrameView {
	...
	readonly View _contentView = new () { Id = "WizardContentView" };
	readonly TextView _helpTextView = new ();
	...
	internal void ShowHide ()
	{
		...
		_contentView.SetDesiredVisibility (_contentView.InternalSubviews?.Count > 0);
		_helpTextView.SetDesiredVisibility (_helpTextView.Text.Length > 0);
	}
