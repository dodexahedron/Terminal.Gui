...
public abstract class Axis {
	...
	string DefaultLabelGetter (AxisIncrementToRender toRender) => toRender.Value.ToString ("N0");
}
...
public class HorizontalAxis : Axis {
	...
	public override void DrawAxisLine (GraphView graph)
	{
		...
		for (var i = xStart; i < bounds.Width; i++) {
	...
	IEnumerable<AxisIncrementToRender> GetLabels (GraphView graph, Rect bounds)
	{
		// if no labels
		if (Increment == 0) {
			yield break;
...
public class VerticalAxis : Axis {
	...
	int GetAxisYEnd (GraphView graph)
	{
	...
	/// set <see cref="GraphView.MarginLeft"/> or <see cref="GraphView.ScrollOffset"/> sufficient that it is visible
	/// </param>
	public override void DrawAxisLabel (GraphView graph, int screenPosition, string text)
	{
		var x = GetAxisXPosition (graph);
