...
public class SpinnerView : View {
	...
	static readonly SpinnerStyle DEFAULT_STYLE = new SpinnerStyle.Line ();
	bool _bounce = DEFAULT_STYLE.SpinBounce;
	bool _bounceReverse;
	int _currentIdx;
	int _delay = DEFAULT_STYLE.SpinDelay;
	DateTime _lastRender = DateTime.MinValue;
	string [] _sequence = DEFAULT_STYLE.Sequence;

	SpinnerStyle _style = DEFAULT_STYLE;
	object _timeout;
	...
	void SetSequence (string [] frames)
	{
	...
	int GetSpinnerWidth ()
	{
		...
		if (_sequence is not null && _sequence.Length > 0) {
			foreach (var frame in _sequence) {
				if (frame.Length > max) {
					max = frame.Length;
				}
			}
	...
	/// </summary>
	/// <remarks>
	/// Ensure this method is called on the main UI
