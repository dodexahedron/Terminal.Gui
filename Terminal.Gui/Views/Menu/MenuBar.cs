...
public class MenuBarItem : MenuItem {
	...
	public MenuBarItem (string title, string help, Action action, Func<bool> canExecute = null,
		MenuItem parent = null) : base (title, help, action, canExecute, parent) =>
	...
	public MenuBarItem SubMenu (MenuItem menuItem) => menuItem as MenuBarItem;
	...
	internal void AddKeyBindings (MenuBar menuBar)
	{
		...
		foreach (var menuItem in Children.Where (m => m != null)) {
			if (menuItem.HotKey != default) {
				...
				menuBar.KeyBindings.Add ((KeyCode)menuItem.HotKey.Value | KeyCode.AltMask,
					KeyBindingScope.HotKey, Command.ToggleExpandCollapse);
			}
...
/// <summary>
///         <para>
///                 Provides a menu bar that spans the top of a <see cref="Toplevel" /> View with drop-down and cascading
///                 menus.
///         </para>
///         <para>
///                 By default, any sub-sub-menus (sub-menus of the <see cref="MenuItem" />s added to
///                 <see cref="MenuBarItem" />s)
///                 are displayed in a cascading manner, where each sub-sub-menu pops out of the sub-menu frame
///                 (either to the right or left, depending on where the sub-menu is relative to the edge of the screen).
///                 By setting
///                 <see cref="UseSubMenusSingleFrame" /> to <see langword="true" />, this behavior can be changed such
///                 that all sub-sub-menus are
///                 drawn within a single frame below the MenuBar.
///         </para>
/// </summary>
/// <remarks>
///         <para>
///                 The <see cref="MenuBar" /> appears on the first row of the <see cref="Toplevel" /> SuperView and uses
///                 the full width.
///         </para>
///         <para>
...
///         </para>
///         <para>
///                 The <see cref="MenuBar" /> provides global hot keys for the application. See
///                 <see cref="MenuItem.HotKey" />.
///         </para>
///         <para>
...
///         </para>
///         <para>
///                 If a key press matches any of the menu item's hot keys or shortcuts, the menu item's action is invoked
///                 or
///                 sub-menu opened.
///         </para>
///         <para>
...
///                 * Un-shifted hot keys defined for the menu bar items will be invoked, only if the menu they belong to
///                 is open (the menu bar item's text is visible).
///                 * Alt-shifted hot keys defined for the menu bar items will be invoked, only if the menu they belong to
///                 is open (the menu bar item's text is visible).
...
public class MenuBar : View {
	...
	static readonly int _xOrigin = 0;

	// Spaces before the Title
	static readonly int _leftPadding = 1;

	// Spaces after the Title
	static readonly int _rightPadding = 1;

	// Spaces after the submenu Title, before Help
	static readonly int _parensAroundHelp = 3;
	...
	public MenuBar ()
	{
		...
		KeyBindings.Add (KeyCode.CtrlMask | KeyCode.Space, KeyBindingScope.HotKey,
		...
		set {
			...
			foreach (var menuBarItem in Menus?.Where (m => m != null)!) {
				if (menuBarItem.HotKey != default) {
					KeyBindings.Add ((KeyCode)menuBarItem.HotKey.Value,
						Command.ToggleExpandCollapse);
					KeyBindings.Add ((KeyCode)menuBarItem.HotKey.Value | KeyCode.AltMask,
						KeyBindingScope.HotKey, Command.ToggleExpandCollapse);
				}
	...
	///         <para>
	///                 By default any sub-sub-menus (sub-menus of the main <see cref="MenuItem" />s) are displayed in a
	///                 cascading manner,
	...
	internal bool SelectEnabledItem (IEnumerable<MenuItem> chldren, int current, out int newCurrent,
		bool forward = true)
	{
		if (chldren == null) {
			newCurrent = -1;
	...
	public bool CloseMenu (bool ignoreUseSubMenusSingleFrame = false) =>
		CloseMenu (false, false, ignoreUseSubMenusSingleFrame);
	...
	void RemoveSubMenu (int index, bool ignoreUseSubMenusSingleFrame = false)
	{
		...
		for (var i = _openSubMenu.Count - 1; i > index; i--) {
			_isMenuClosing = true;
			Menu menu;
	...
	/// <remarks>
	///         <para>
	///                 If the user presses any <see cref="MenuItem.HotKey" />s defined in the <see cref="MenuBarItem" />s, the
	///                 menu bar will be activated and the sub-menu will be opened.
	///         </para>
	///         <para>
	...
	bool _useKeysUpDownAsKeysLeftRight;
	...
	static Rune _shortcutDelimiter = new ('+');
