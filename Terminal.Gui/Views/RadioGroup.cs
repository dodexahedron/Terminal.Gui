...
public class RadioGroup : View {
	...
	/// Initializes a new instance of the <see cref="RadioGroup"/> class using <see cref="LayoutStyle.Computed"/> layout.
	/// </summary>
	/// <param name="radioLabels">
	/// The radio labels; an array of strings that can contain hotkeys using an underscore before the
	/// letter.
	/// </param>
	/// <param name="selected">The index of the item to be selected, the value is clamped to the number of items.</param>
	public RadioGroup (string [] radioLabels, int selected = 0) => SetInitialProperties (radioLabels, selected);
	...
	public RadioGroup (Rect rect, string [] radioLabels, int selected = 0) : base (rect) => SetInitialProperties (radioLabels, selected);
	...
	void RadioGroup_LayoutStarted (object sender, EventArgs e) => SetWidthHeight (_radioLabels);
	...
	public override void OnDrawContent (Rect contentArea)
	{
		...
		for (var i = 0; i < _radioLabels.Count; i++) {
			...
			TextFormatter.FindHotKey (rl, HotKeySpecifier, out var hotPos, out var hotKey);
	...
	void SelectItem () => SelectedItem = _cursor;

	void MoveEnd () => _cursor = Math.Max (_radioLabels.Count - 1, 0);
