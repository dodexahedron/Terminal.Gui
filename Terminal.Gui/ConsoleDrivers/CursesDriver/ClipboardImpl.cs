...
#pragma warning disable RCS1075 // Avoid empty catch clause that catches System.Exception.
		try {
			(var exitCode, var result) = ClipboardProcessRunner.Bash ("which xclip", waitForOutput: true);
...
#pragma warning restore RCS1075 // Avoid empty catch clause that catches System.Exception.
	...
	protected override string GetClipboardDataImpl ()
	{
		...
		try {
			(var exitCode, var result) = ClipboardProcessRunner.Bash ($"{_xclipPath} {xclipargs} > {tempFileName}", waitForOutput: false);
			if (exitCode == 0) {
	...
	protected override void SetClipboardDataImpl (string text)
	{
		...
		try {
			(var exitCode, var _) = ClipboardProcessRunner.Bash ($"{_xclipPath} {xclipargs}", text);
...
class MacOSXClipboard : ClipboardBase {
	readonly IntPtr _allocRegister = sel_registerName ("alloc");
	readonly IntPtr _clearContentsRegister = sel_registerName ("clearContents");
	readonly IntPtr _generalPasteboard;
	readonly IntPtr _generalPasteboardRegister = sel_registerName ("generalPasteboard");
	readonly IntPtr _initWithUtf8Register = sel_registerName ("initWithUTF8String:");
	readonly IntPtr _nsPasteboard = objc_getClass ("NSPasteboard");
	readonly IntPtr _nsString = objc_getClass ("NSString");
	readonly IntPtr _nsStringPboardType;
	readonly IntPtr _setStringRegister = sel_registerName ("setString:forType:");
	readonly IntPtr _stringForTypeRegister = sel_registerName ("stringForType:");
	readonly IntPtr _utf8Register = sel_registerName ("UTF8String");
	readonly IntPtr _utfTextType;
	...
	bool CheckSupport ()
	{
		(var exitCode, var result) = ClipboardProcessRunner.Bash ("which pbcopy", waitForOutput: true);
		if (exitCode != 0 || !result.FileExists ()) {
			return false;
		}
	...
	extern static IntPtr objc_getClass (string className);

	[DllImport ("/System/Library/Frameworks/AppKit.framework/AppKit")]
	extern static IntPtr objc_msgSend (IntPtr receiver, IntPtr selector);

	[DllImport ("/System/Library/Frameworks/AppKit.framework/AppKit")]
	extern static IntPtr objc_msgSend (IntPtr receiver, IntPtr selector, string arg1);

	[DllImport ("/System/Library/Frameworks/AppKit.framework/AppKit")]
	extern static IntPtr objc_msgSend (IntPtr receiver, IntPtr selector, IntPtr arg1);

	[DllImport ("/System/Library/Frameworks/AppKit.framework/AppKit")]
	extern static IntPtr objc_msgSend (IntPtr receiver, IntPtr selector, IntPtr arg1, IntPtr arg2);

	[DllImport ("/System/Library/Frameworks/AppKit.framework/AppKit")]
	extern static IntPtr sel_registerName (string selectorName);
}
...
class WSLClipboard : ClipboardBase {
	...
	static string _powershellPath = string.Empty;
	...
	bool CheckSupport ()
	{
		if (string.IsNullOrEmpty (_powershellPath)) {
			...
			(var exitCode, var result) = ClipboardProcessRunner.Bash ("which pwsh.exe", waitForOutput: true);
			if (exitCode > 0) {
	...
	protected override string GetClipboardDataImpl ()
	{
		...
		(var exitCode, var output) = ClipboardProcessRunner.Process (_powershellPath, "-noprofile -command \"Get-Clipboard\"");
		if (exitCode == 0) {
			if (Application.Driver is CursesDriver) {
				Curses.raw ();
				Curses.noecho ();
	...
	protected override void SetClipboardDataImpl (string text)
	{
		...
		(var exitCode, var output) = ClipboardProcessRunner.Process (_powershellPath, $"-noprofile -command \"Set-Clipboard -Value \\\"{text}\\\"\"");
		if (exitCode == 0) {
			if (Application.Driver is CursesDriver) {
				Curses.raw ();
				Curses.noecho ();
